"""
statistical_tests_icu24h.py — Statistical significance testing for the
Q-model FP/FN reduction, using the CONFIRMED best (threshold, strategy,
model_type) per base model. Does NOT retrain anything: loads the
Q-models already trained and saved by
qmodel_feature_importance_shap_icu24h.py (results/icu24h/artifacts/qmodels/),
then evaluates them on the TEST split only.
...
Output: statistical_tests_summary_icu24h.csv in ARTIFACTS_DIR, one row
per base model, with all three test results.
"""
...
def get_q_probs(base_model, cfg, X_test_q):
    ...
    how qmodel_sweep_*.py originally combined CrossFit predictions)."""
    prefix = base_model.replace(' ', '').lower()
...
# ============================================================
# Confirmed best config per base model (icu24h)
# — identical to the SHAP script's CONFIGS
# ============================================================
CONFIGS = {
    'BasicMLP': dict(
        thr=0.09, strategy='Simple', qtype='MLP', q_thr=0.83,
...
EXTRA_COL_KEYS = {
    'prob':  ('test_prob_icu', 'prob_icu24h'),
...
out_path = os.path.join(ARTIFACTS_DIR, "statistical_tests_summary_icu24h.csv")
