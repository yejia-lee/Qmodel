"""
cali.py — Standalone calibration script for the BasicMLP base model.
...
and saves everything needed by qmodel_sweep.py into a single .npz file,
so the sweep script never needs to reload the model or rerun inference.
"""
...
val_prob_icu_platt   = platt.predict_proba(val_prob_icu.reshape(-1, 1))[:, 1]   # NEW

train_prob_icu_iso = iso.predict(train_prob_icu)
test_prob_icu_iso  = iso.predict(test_prob_icu)
val_prob_icu_iso   = iso.predict(val_prob_icu)   # NEW
...
diag_path = os.path.join(CSV_DIR, "calibration_diagnostics_train_test.csv")
pd.DataFrame([{
...
# ============================================================
# 5. Save everything qmodel_sweep.py needs
# ============================================================
np.savez(
    NPZ_OUT,
    ...
    val_prob_icu_platt=val_prob_icu_platt,        # NEW
    train_prob_icu_iso=train_prob_icu_iso, test_prob_icu_iso=test_prob_icu_iso,
    val_prob_icu_iso=val_prob_icu_iso,            # NEW
)
