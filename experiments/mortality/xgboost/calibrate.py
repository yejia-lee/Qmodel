"""
cali_xgb_mortality365d.py — Standalone base-model + calibration script for XGBoost (365d mortality).

Trains the XGBoost base model (mask-included feature space, 365d
mortality target only), computes train/val/test probabilities, fits
Platt + Isotonic calibrators on VAL only, applies them to TRAIN/TEST,
and saves everything needed by qmodel_sweep_xgb_mortality365d.py into a
single .npz file so the sweep script never needs to retrain the base
XGBoost model.

ASSUMPTION: BASE_DIR points to the same folder used by
xgboost_mortality365d.py (only used here for output paths — XGBoost has
no checkpoint file, so the base model is retrained inside this script,
...
print(f"Features (mask included): {len(all_features_with_mask)} "
...
diag_path = os.path.join(CSV_DIR, "calibration_diagnostics_xgb_mortality365d.csv")
pd.DataFrame([{
...
print("cali_xgb_mortality365d.py done.")