"""
cali_basicmlp_mortality365d.py — Standalone calibration script for the BasicMLP base model (365d mortality).

Runs the full pipeline (data load -> preprocessing -> pretrained model
inference -> Platt/Isotonic calibration fit on VAL -> applied to TRAIN/TEST)
and saves everything needed by qmodel_sweep_basicmlp_mortality365d.py into
a single .npz file, so the sweep script never needs to reload the model
or rerun inference.

ASSUMPTION: this script lives in the same directory as
basicmlp_mortality365d.py (so BASE_DIR resolves to the same folder that
holds best_basicmlp_mortality365d_only.pt). If that's not where the
checkpoint is, update BASE_DIR below.
"""
...
NPZ_OUT     = os.path.join(CSV_DIR, "calibrated_probs_basicmlp_mortality365d.npz")
...
# ============================================================
# Model definitions (identical to base training script)
# ============================================================
...
# ============================================================
# 1. Load & preprocess data (identical to base training script)
# ============================================================
print("\nLoading data...")
...
base_train_prob_icu = base_train_probs[:, MORTALITY_IDX]
base_train_true_icu = base_train_labels[:, MORTALITY_IDX]
...
train_prob_icu = train_probs[:, MORTALITY_IDX]
train_true_icu = train_labels[:, MORTALITY_IDX]
...
val_prob_icu = val_probs[:, MORTALITY_IDX]
val_true_icu = val_labels[:, MORTALITY_IDX]
val_mask     = ~np.isnan(val_true_icu)
val_prob_icu = val_prob_icu[val_mask]
val_true_icu = val_true_icu[val_mask].astype(int)

test_prob_icu = test_probs[:, MORTALITY_IDX]
test_true_icu = test_labels[:, MORTALITY_IDX]
...
diag_path = os.path.join(CSV_DIR, "calibration_diagnostics_basicmlp_mortality365d.csv")
pd.DataFrame([{
...
# ============================================================
# 5. Save everything qmodel_sweep_basicmlp_mortality365d.py needs
# ============================================================
np.savez(
    NPZ_OUT,
...
print("cali_basicmlp_mortality365d.py done.")