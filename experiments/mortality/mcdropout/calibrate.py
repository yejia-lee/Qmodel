"""
cali_mc_mortality365d.py — Standalone calibration script for the MC Dropout base model (365d mortality).
...
needed by qmodel_sweep_mc_mortality365d.py into a single .npz file.

ASSUMPTION: this script lives in the same directory as
mcdropout_mortality365d.py (so BASE_DIR resolves to the folder that
holds best_mcdropout_mortality365d_only_mask.pt). If not, update
BASE_DIR below.
"""
...
# ============================================================
# 1. Load & preprocess data (identical to MC Dropout training script)
# ============================================================
print("\nLoading data...")
...
base_train_prob_icu = base_train_mean[:, MORTALITY_IDX][base_train_mask]
base_train_true_icu = base_train_labels[:, MORTALITY_IDX][base_train_mask].astype(int)
...
train_prob_icu = train_mean[:, MORTALITY_IDX][train_mask]
train_var_icu  = train_var[:, MORTALITY_IDX][train_mask]
train_std_icu  = np.sqrt(train_var_icu)
train_ent_icu  = train_ent[:, MORTALITY_IDX][train_mask]
train_true_icu = train_labels[:, MORTALITY_IDX][train_mask].astype(int)

val_prob_icu = val_mean[:, MORTALITY_IDX][val_mask]
val_var_icu  = val_var[:, MORTALITY_IDX][val_mask]
val_std_icu  = np.sqrt(val_var_icu)
val_ent_icu  = val_ent[:, MORTALITY_IDX][val_mask]
val_true_icu = val_labels[:, MORTALITY_IDX][val_mask].astype(int)

test_prob_icu = test_mean[:, MORTALITY_IDX][test_mask]
test_var_icu  = test_var[:, MORTALITY_IDX][test_mask]
test_std_icu  = np.sqrt(test_var_icu)
test_ent_icu  = test_ent[:, MORTALITY_IDX][test_mask]
test_true_icu = test_labels[:, MORTALITY_IDX][test_mask].astype(int)
...
diag_path = os.path.join(CSV_DIR, "calibration_diagnostics_mc_mortality365d.csv")
pd.DataFrame([{
...
# ============================================================
# 5. Save everything qmodel_sweep_mc_mortality365d.py needs
# ============================================================
np.savez(
    NPZ_OUT,
