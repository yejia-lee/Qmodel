"""Threshold-specific FP-risk Q-model selection for 365d Mortality (MC Dropout)."""
...
SUMMARY_CSV_NAME = "prob_thr_sweep_summary_mcdropout_mortality365d_only_mask_trainQ_WITH_CALIBRATION.csv"
...
def q_features(data, split):
    return np.hstack([
        ...
        data[split + "_var"][:, None], data[split + "_ent"][:, None],
    ]).astype(np.float32)
